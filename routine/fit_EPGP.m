function [ fit ] = fit_EPGP( P, Y, regr, corr, lob, upb, theta0 )
% Periodic Gaussian Process
    ...
    if nargin == 7
        [f, fit] = objfunc(theta0, data);
       	...
       	% init dataameters
        u = linspace(0,1,5);    [x1, x2, x3] = meshgrid(u(2:end-1), u(2:end-1), u(2:end-1));
...
function [obj, fit] = objfunc(para, data) % using smt package
    delta = para(1);    theta = para(2);    rho = para(3);% Initialize
    ...
    for i = 1 : length(data.P)
        p = data.P(i)';
        r = data.corr(rho, theta, p, (1:n)', 1);    r(1) = r(1) + delta^2;
        L = toep_chol(r);
        Ft = L \ data.F;    Yt = L \ data.Y; 
        [Q, G] = qr(Ft,0);    B = G \ (Q'*Yt);
        ro = Yt - Ft*B;     sigma2 = sum(ro.^2)/n;
