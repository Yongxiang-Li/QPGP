function [ fit ] = fit_QPGP( P, Y, regr, corr, lob, upb, theta0 )
...
function [obj, fit] = objfunc(para, data) % using smt package
    ...
    for i = 1 : length(data.P)
        p = data.P(i);    k = floor(n/p);    p1 = n - p*k;
        Y = zeros(p,k);    Y(1:k*p) = data.Y(1:k*p);    Ys = data.Y(k*p+1:n);
        Gamma = data.regr((1:p)');    Gs = Gamma(1:p1,:);
        ...
        if p1 == 0
            ...
            sigma2 = (ChiYY - Beta'*ChiFF*Beta)/n; % sigma
            likelihood(i) = (n*log(sigma2) + sum(sum(log(real(S)))) + n + n*log(2*pi))/2;
        else
            ...
            Fd = ifft((Sr.*(Lambda*(UoW.*UoOnes))).*fft(Gamma));
            Fd = real(Gs - Fd(1:p1,:)); % Fd = Fs - real(UrRd'*((Lambda*(UoW.*UoOnes)).*UrGamma));
            L = toep_chol(r(1:p1)); % S = chol((Pi+eps*eye(size(Pi))), 'lower');
            Fdl = L \ Fd;    FPiF = Fdl'*Fdl;
            Ydl = L \ Yd;    YPiY = Ydl'*Ydl;
            FPiY = Fdl'*Ydl;
            Beta = (ChiFF + FPiF) \ (ChiFY + FPiY); % beta
            sigma2 = ((ChiYY+YPiY) - Beta'*(ChiFF+FPiF)*Beta)/n; % sigma
            likelihood(i) = (n*log(sigma2) + sum(sum(log(real(S)))) + sum(2*log(diag(L))) + n + n*log(2*pi))/2;
        end
        sigma(i) = sqrt(sigma2);
        beta(i,:) = Beta;
